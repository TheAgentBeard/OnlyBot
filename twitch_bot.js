...
async sayTTS(channel, text, userstate) {
    ...
    try {
        ...
        const response = await axios.post('https://api.elevenlabs.io/v1/text-to-speech', {
            text: text,
            voice: 'en_us_male', // Wähle eine Stimme aus, z.B. 'en_us_male'
            model: 'eleven_monolingual_v1' // Standardmodell
        }, {
            headers: {
                'Authorization': `sk_5a7d1e99d43df9ba5247ddd33f55d464eac838df6d4705f3`, // Ersetze 'YOUR_API_KEY' mit deinem echten API-Key
                'Content-Type': 'application/json'
            },
            responseType: 'arraybuffer' // Erwarte eine MP3-Datei im Antwortkörper
        });

        // Konvertiere die erhaltene Antwort in einen Buffer
        const buffer = Buffer.from(response.data);

        // Speichern der MP3-Datei auf dem Server
        const filePath = './public/file.mp3';
        await fsPromises.writeFile(filePath, buffer);

        // Rückgabe des Pfades zur gespeicherten Audiodatei
        return filePath;

    } catch (error) {
        console.error('Fehler bei der TTS-Verarbeitung:', error);
    }
}
